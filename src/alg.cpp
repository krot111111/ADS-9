...
PMTree::Node* PMTree::generate(const std::vector<char>& available) {
    if (available.empty()) return new Node(0);
...
void collectPaths(PMTree::Node* rootNode, std::vector<char>& sequence,
    std::vector<std::vector<char>>& collection, int level, int limit) {
    if (level == limit) {
...
std::vector<char> getPerm1(PMTree& treeObj, int num) {
    ...
    if (num <= 0 || num > static_cast<int>(storage.size())) return {};
    return storage[num - 1];
}

size_t fact(int n) {
    size_t totalFact = 1;
    for (int idx = 2; idx <= n; ++idx) totalFact *= idx;
    return totalFact;
}

std::vector<char> getPerm2(PMTree& treeObj, int num) {
    int sizeCount = treeObj.base.size();
    if (num <= 0 || static_cast<size_t>(num) > fact(sizeCount)) return {};
